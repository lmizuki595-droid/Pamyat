...
function checkAndTrimMessages() {
    try {
        // Загружаем лимит каждый раз актуальный
        ...
        const context = window.SillyTavern?.context || (typeof getContext === 'function' ? getContext() : null);
...
function createUI() {
    try {
        ...
        // Если вкладка уже была создана случайно, не дублируем её
...
// Инициализация через jQuery, когда DOM полностью готов
jQuery(function () {
    createUI();
    
    // Подключаемся к событиям таверны напрямую через глобальный объект, если импорты отвалились
    ...
    if (eventSource && event_types) {
        ...
        // Запасной вариант: следим за изменениями в чате через стандартный MutationObserver браузера
