(function () {
    // Функция расчета токенов и скрытия
    function checkAndTrimMessages() {
        try {
            // Берем контекст напрямую из правильного места модуля SillyTavern
            ...
            console.error("[Trimmer] Ошибка расчета:", err);
        }
    }

    // Создаем разметку, которую SillyTavern свернет САМА автоматическими инструментами
    function createUI() {
        const extensionsMenu = document.getElementById('extensions_settings');
        if (!extensionsMenu || document.getElementById('context-trimmer-container')) return;
        ...
        // Мы убрали ручные стили display: block. Теперь CSS таверны сам управляет скрытием!
        container.innerHTML = `
            ...
            <div class="inline-drawer-content trimmer-settings-block">
    ...
    // Ждем полной готовности API самой Таверны
    function init() {
        createUI();
        ...
        // Подстраховка на любые изменения чата
    ...
    // Запускаем инициализацию с небольшой задержкой, чтобы ядро ST успело встать в window
